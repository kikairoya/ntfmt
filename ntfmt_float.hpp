...
namespace ntfmt {
	namespace detail {
		...
		template <typename T>
		inline T get_predecessor(T const &v) { return nextafter(v, -std::numeric_limits<T>::infinity()); }
		template <typename T>
		inline T get_successor(T const &v) { return nextafter(v, +std::numeric_limits<T>::infinity()); }
		...
		template <typename T>
		struct dtoa_traits {
			typedef typename boost::int_max_value_t<std::numeric_limits<T>::max_exponent>::least higher_type;
			typedef typename boost::int_min_value_t<std::numeric_limits<T>::min_exponent>::least lower_type;
		...
		typename dtoa_traits<T>::return_type dtoa(char *const out, T v, int prec, unsigned const base) {
			...
			v = std::abs(v);
			return_type const k = detail::ilog(base, detail::get_successor(v));
			
			char buf[std::numeric_limits<T>::digits + 3] = "0";
			if (prec >= std::numeric_limits<T>::digits-3) prec = std::numeric_limits<T>::digits-3;
			{
				T q = v / detail::ipow<double>(base, k);
				for (size_t i=1; i<sizeof(buf)-1; ++i) {
					q *= base;
					T const fq = std::floor(q);
					buf[i] = detail::hexstr[static_cast<unsigned>(fq)%base];
					q -= fq;
				}
			}
			if ((unsigned)(buf[prec+1]-'0') >= base/2) detail::inc_strnum(buf, base, prec);
		...
		inline bool isinf(long double d) { return !_finite(d); }
		inline bool isnan(float d) { return _isnan(d); }
		inline bool isnan(double d) { return _isnan(d); }
		inline bool isnan(long double d) { return _isnan(d); }
		inline float nextafter(float x, float y) { return _nextafter(x, y); }
		inline double nextafter(double x, double y) { return _nextafter(x, y); }
		inline long double nextafter(long double x, long double y) { return _nextafter(x, y); }
#endif
		template <typename T>
		void float_printer(sink_fn_t &fn, T const &value, const flags_t &flags) {
			...
			if (unlikely(isinf(value))) {
				strcpy(phead, flags.capital ? "INF" : "inf");
				fn(head);
				return;
			}
			int const prec = flags.prec_enable ? flags.precision : 6;
			int const l = ilog(flags.radix, std::abs(value));
			bool const exponential = flags.exponential || ( !flags.fixed && ((l < -4) || (l > prec)));
			char buf[std::numeric_limits<T>::digits10 + 8];
			if (exponential) {
